...
async def on_message(message):
  ...
  if 'Discord' in message.content:
    await message.channel.send(f'Gin comunnity https://Discord.gg/T4reMuNx')
  if "What's Gin'ebra?" in message.content:
...
async def av (ctx, member: discord.Member = None):
  ...
  avatar=discord.Embed(title=f"{member.name}'s avatar",
                      color=0x000000,
...
@bot.event
async def on_ready():
  await bot.change_presence(status=discord.Status.idle, activity=discord.Activity(type=discord.ActivityType.watching, name="Gin'ebra movie"))
...
bot.run('OTUzMzU3ODY5ODM3ODUyNzAz.YjDZtA.sc_6syPBk55mrGvMKMVNbqVdy40')